...
const Support: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-4 font-mono relative md:h-[calc(100dvh-12rem)] md:min-h-0 md:overflow-hidden">
      ...
      <Marquee speed={40} className="bg-primary text-primary-foreground border-none py-0.5 font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-1 gap-4 md:flex-1 md:min-h-0 md:overflow-hidden">
        ...
        <div className="hidden md:flex flex-col gap-4 md:min-h-0">
          <Button
            ...
            className="w-full bg-foreground text-background hover:bg-foreground/90 uppercase text-xs tracking-widest gap-2 h-9"
          >
            <Plus className="h-4 w-4" /> {i18n.language === 'bn' ? 'নতুন চ্যাট' : 'New Chat'}
          </Button>

          <Card className="bg-card/50 border-border flex-1 min-h-0 flex flex-col">
        ...
        {/* Main Chat Area */}
        <Card className="md:col-span-3 h-[calc(100dvh-13rem)] md:h-full flex flex-col bg-background border-border shadow-[0_0_20px_hsl(var(--foreground)/0.05)]">
          <CardHeader className="border-b border-border py-3 flex-row items-center justify-between">
            ...
            <div className="md:hidden flex items-center gap-1">
              <Button
                ...
                size="sm"
                variant="ghost"
                className="h-7 px-2 text-[10px] uppercase tracking-widest gap-1 text-foreground"
              >
          ...
          <CardFooter className="p-4 border-t border-border bg-background flex-col gap-2 items-stretch">
            {token && (
              <Button
                type="button"
            ...
            <form className="flex w-full gap-2" onSubmit={(e) => { e.preventDefault(); handleSend(); }}>
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  disabled={loading}
                  className="pl-7 bg-transparent border-border focus:border-foreground focus:ring-0 uppercase placeholder:opacity-30"
                />
              </div>
              <AsyncButton type="submit" size="icon" aria-label="send" loading={loading} disabled={!input.trim()} className="bg-foreground text-background hover:bg-foreground/90">
      ...
      {historyOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <div className="absolute left-0 top-0 h-full w-[80%] max-w-xs bg-background border-r border-border flex flex-col shadow-[0_0_30px_hsl(var(--background)/0.8)]">
