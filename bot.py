...
- Gift Code and Giveaway: admins can create one-time gift codes with network, amount, and expiry; users redeem with a destination wallet. Giveaways can have expiry and claim limits and deliver configured crypto after successful claim.
- Personal wallet: /setup lets a user connect an encrypted wallet key for supported networks, then use /mybalance and /send_wallet with password protection. /changekey replaces the saved key and /deletekey removes it. The bot cannot recover a forgotten password. Never ask for or reveal private keys, seed phrases, mnemonics, or wallet passwords; support/admin should never ask for them either.
...
- Referral: /referral gives referral link/code and referral balance when enabled. Eligible completed orders can credit referrals. Admin can configure percent and minimum withdrawal. Payout/withdraw request flow is available only when enabled and eligible.
- Rates and balances: Rates screen shows current BDT rates. Admin can set network rates and cost rates. Balance/stock checks account for active reservations so shown available stock can be lower than raw wallet balance. Low stock or low gas warnings mean orders may fail or need admin top-up.
- TX Log and failed sends: TX Log shows user-visible transaction history. Failed delivery can be caused by low stock, low native gas, RPC timeout, invalid wallet/network, chain revert, missing sender/API/RPC config, duplicate TrxID, or seller setup issue. Admin should check /failed, /audit, /balances, /gas, payment receipt, explorer, and retry only after confirming no duplicate send happened.
- Maintenance and health: maintenance mode pauses orders temporarily. /bot_health and /webhook_health help admins check bot/webhook state. bKash webhook stale warnings mean payment notices may be delayed and orders can remain pending.
...
- Recommended answer style: first answer the user's direct question, then give exact steps or likely reasons. If information is missing, ask only for the minimum needed identifier (Order ID or TrxID) and avoid guessing. Keep answers short, practical, and beginner-friendly.
""".strip()
...
def ai_support_prompt(lang="bn"):
    return (
        "You are the read-only AI support assistant for the Mouno Telegram crypto app/bot. "
