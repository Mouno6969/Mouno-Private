...
- AI Support button using first-priority NVIDIA Build/NIM Kimi K2.6, then NVIDIA DeepSeek/Gemma, Gemini, Groq, OpenRouter, Hugging Face, Cohere, or Mistral API keys with automatic provider fallback
...
- Admins can configure AI API keys from Admin Menu → ⚙️ AI Setup and monitor per-model success/failure counts from 📊 AI Usage; keys saved this way are stored in local SQLite `mouno.db` and take effect immediately. `.env` AI keys remain supported as fallback/configuration.
