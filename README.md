...
- AI Support button using Gemini, Groq, or OpenRouter API keys with automatic provider fallback
...
- New order/admin tools: `/order ORD-XXXXXX`, `/status TRXID_OR_ORDERID`, `/receipt ORD_OR_TRX`, `/seller USER_ID`, `/seller_badge USER_ID new|verified|trusted`, `/seller_dashboard`, `/report [weekly]`, `/profit [daily|weekly]`, `/costrate [NETWORK RATE]`, `/gas`, `/reservations`, `/audit`, `/payout`, `/payouts`, `/webhook_health`, `/test_sms`, `/test_seller_sms`, `/aiadmin why order failed ORD-123`.
