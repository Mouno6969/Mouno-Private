...
# ---------------------------------------------------------------------------
# Users can connect many wallets across any network (including several on the
# same network). Every wallet's private key is encrypted with ONE master
# password per user (reusing the Fernet/PBKDF2 scheme above). Balances are read
# from the stored public address and never require the master password; only
# sending decrypts a key transiently.

_MASTER_VERIFIER_PLAINTEXT = "MOUNO_MASTER_VERIFIER_OK"


def init_user_wallets_multi():
    with closing(sqlite3.connect(DB_PATH)) as con:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS user_wallets_multi (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id TEXT NOT NULL,
                network TEXT NOT NULL,
                label TEXT,
        ...
        con.execute("CREATE INDEX IF NOT EXISTS idx_user_wallets_multi_user ON user_wallets_multi(user_id)")
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS wallet_master (
                user_id TEXT PRIMARY KEY,
        ...
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS legacy_wallet_migrated (
                user_id TEXT PRIMARY KEY,
...
def _migrate_legacy_wallets():
    """Copy any existing single-wallet rows into the multi-wallet table.

    Idempotent and non-destructive: the old ``user_wallets`` table is left
    untouched, and each user is migrated at most once (tracked in
    ``legacy_wallet_migrated``) so wallets the user later deletes do not
    resurrect on the next startup.
    """
    with closing(sqlite3.connect(DB_PATH)) as con:
        try:
            ...
            return  # legacy table does not exist yet

        for user_id, encrypted_key, salt, network, wallet_address in legacy_rows:
            if not user_id or not encrypted_key or not salt:
                continue
            already = con.execute(
                "SELECT 1 FROM legacy_wallet_migrated WHERE user_id=?", (str(user_id),)
            ).fetchone()
            if already:
                continue
            con.execute(
                """
                INSERT INTO user_wallets_multi
                (user_id, network, label, wallet_address, encrypted_key, salt)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (str(user_id), network or "solana", None, wallet_address, encrypted_key, salt),
            )
            con.execute(
                "INSERT OR IGNORE INTO legacy_wallet_migrated (user_id) VALUES (?)",
                (str(user_id),),
            )
        con.commit()
...
def has_master_password(user_id) -> bool:
    with closing(sqlite3.connect(DB_PATH)) as con:
        return con.execute(
            "SELECT 1 FROM wallet_master WHERE user_id=?", (str(user_id),)
        ).fetchone() is not None


def set_master_password(user_id, password):
    verifier, salt = encrypt_key(_MASTER_VERIFIER_PLAINTEXT, password)
    with closing(sqlite3.connect(DB_PATH)) as con:
        con.execute(
            "INSERT OR REPLACE INTO wallet_master (user_id, verifier, salt) VALUES (?, ?, ?)",
            (str(user_id), verifier, salt),
...
def verify_master_password(user_id, password) -> bool:
    """Validate a user's master password.

    If a verifier already exists, check against it. Otherwise (e.g. a user
    migrated from the old single-wallet scheme) try to decrypt one of their
    existing wallet keys; on success, lazily store the verifier so subsequent
    checks are cheap.
    """
    uid = str(user_id)
    with closing(sqlite3.connect(DB_PATH)) as con:
        row = con.execute(
            "SELECT verifier, salt FROM wallet_master WHERE user_id=?", (uid,)
        ).fetchone()
    if row:
        try:
            return decrypt_key(row[0], row[1], password) == _MASTER_VERIFIER_PLAINTEXT
        except Exception:
            return False

    # No verifier yet — try lazy adoption against an existing wallet key.
    with closing(sqlite3.connect(DB_PATH)) as con:
        wallet = con.execute(
            "SELECT encrypted_key, salt FROM user_wallets_multi WHERE user_id=? ORDER BY id LIMIT 1",
            (uid,),
        ).fetchone()
    if not wallet:
        return False
    try:
        decrypt_key(wallet[0], wallet[1], password)
    except Exception:
        return False
    set_master_password(uid, password)
    return True


# ─── Key generation (create mode) ───

def generate_private_key(network):
    """Generate a brand new private key for the given network."""
    ...
    if network == "trc20":
        ...
        return PrivateKey.random().hex()
    if network == "ton":
        raise RuntimeError("Creating new TON wallets is not supported. Import an existing key instead.")
...
def add_user_wallet(user_id, network, private_key_or_create, master_password, label=None):
    """Create or import a wallet for ``network`` and store it encrypted.

    ``private_key_or_create`` is either a private key string (import) or the
    sentinel ``"create"`` to generate a fresh wallet. On the user's very first
    wallet this sets the master password; afterwards the master password is
    verified (and adopted for migrated users) before storing.
    """
    uid = str(user_id)
    if private_key_or_create == "create":
        private_key = generate_private_key(network)
    else:
        private_key = (private_key_or_create or "").strip()
        if not private_key:
            raise RuntimeError("Private key is required to import a wallet.")

    wallet_address = get_wallet_address(network, private_key)
    if not wallet_address:
        raise RuntimeError("Invalid private key for this network.")

    existing = list_user_wallets(uid)
    if has_master_password(uid) or existing:
        if not verify_master_password(uid, master_password):
            raise RuntimeError("Wrong master password.")
    else:
        # Truly the first wallet for this user — set the master password.
        set_master_password(uid, master_password)
    ...
    with closing(sqlite3.connect(DB_PATH)) as con:
        cur = con.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (uid, network, label, wallet_address, encrypted_key, salt),
        )
        con.commit()
        wallet_id = cur.lastrowid
    return {"id": wallet_id, "network": network, "label": label, "wallet_address": wallet_address}
...
def list_user_wallets(user_id):
    """Return all of a user's wallets without decrypting any keys."""
    with closing(sqlite3.connect(DB_PATH)) as con:
        rows = con.execute(
            "SELECT id, network, label, wallet_address, created_at FROM user_wallets_multi WHERE user_id=? ORDER BY id",
            (str(user_id),),
        ).fetchall()
    return [
        {"id": r[0], "network": r[1], "label": r[2], "wallet_address": r[3], "created_at": r[4]}
        for r in rows
    ]
...
def get_user_wallets_by_network(user_id, network):
    with closing(sqlite3.connect(DB_PATH)) as con:
        rows = con.execute(
            "SELECT id, network, label, wallet_address FROM user_wallets_multi WHERE user_id=? AND network=? ORDER BY id",
            (str(user_id), network),
        ).fetchall()
    return [{"id": r[0], "network": r[1], "label": r[2], "wallet_address": r[3]} for r in rows]


def get_wallet_record(user_id, wallet_id):
    """Internal: fetch a single wallet row scoped to the owning user."""
    with closing(sqlite3.connect(DB_PATH)) as con:
        return con.execute(
            "SELECT id, network, label, wallet_address, encrypted_key, salt FROM user_wallets_multi WHERE id=? AND user_id=?",
...
def delete_user_wallet_by_id(user_id, wallet_id, master_password=None):
    uid = str(user_id)
    if master_password is not None and not verify_master_password(uid, master_password):
        raise RuntimeError("Wrong master password.")
    with closing(sqlite3.connect(DB_PATH)) as con:
        cur = con.execute(
            "DELETE FROM user_wallets_multi WHERE id=? AND user_id=?", (wallet_id, uid)
        )
        con.commit()
...
def send_from_wallet(user_id, wallet_id, master_password, to_address, amount, asset=None):
    """Decrypt a specific wallet's key (verifying the master password) and send."""
    record = get_wallet_record(user_id, wallet_id)
    if not record:
        raise RuntimeError("Wallet not found.")
    _id, network, _label, _addr, encrypted_key, salt = record
    try:
        private_key = decrypt_key(encrypted_key, salt, master_password)
    except Exception as exc:
        raise RuntimeError("Wrong master password.") from exc

    send_network = network
    if asset and network == "solana" and str(asset).lower() in ("usdt", "solana_usdt"):
        send_network = "solana_usdt"
    return send_with_private_key(send_network, private_key, to_address, float(amount))


# ─── Legacy single-wallet API (now delegating to the multi-wallet table) ───
# These keep older call sites in bot.py / app.py working by operating on the
# user's primary (first) wallet stored in user_wallets_multi.

def save_user_wallet(user_id, encrypted_key, salt, network, wallet_address):
    """Append an already-encrypted wallet for legacy callers."""
    with closing(sqlite3.connect(DB_PATH)) as con:
        con.execute(
            """
            INSERT INTO user_wallets_multi
...
def get_user_wallet(user_id):
    """Return the user's primary (first) wallet as (encrypted_key, salt, network, address)."""
    with closing(sqlite3.connect(DB_PATH)) as con:
        return con.execute(
            "SELECT encrypted_key, salt, network, wallet_address FROM user_wallets_multi WHERE user_id=? ORDER BY id LIMIT 1",
            (str(user_id),),
        ).fetchone()
...
def get_user_evm_wallet(user_id):
    with closing(sqlite3.connect(DB_PATH)) as con:
        return con.execute(
            "SELECT encrypted_key, salt, network, wallet_address FROM user_wallets_multi WHERE user_id=? AND network NOT LIKE 'solana%' AND network != 'trc20' AND network != 'ton' ORDER BY id LIMIT 1",
            (str(user_id),),
        ).fetchone()
...
def get_user_solana_wallet(user_id):
    with closing(sqlite3.connect(DB_PATH)) as con:
        return con.execute(
            "SELECT encrypted_key, salt, network, wallet_address FROM user_wallets_multi WHERE user_id=? AND network LIKE 'solana%' ORDER BY id LIMIT 1",
            (str(user_id),),
        ).fetchone()


def delete_user_wallet(user_id):
    """Legacy delete — removes ALL of the user's wallets (old single-wallet semantics)."""
    with closing(sqlite3.connect(DB_PATH)) as con:
        con.execute("DELETE FROM user_wallets_multi WHERE user_id=?", (str(user_id),))
        con.commit()
